      ...
      {movie?.map((m) => (
        ...
        >
          {m?.reviews.map((review) => (
            <div
              key={review._id}
              className="bg-[#496989] border border-black-500 p-4 rounded-lg w-[50%] mt-[2rem]"
            >
              <div className="flex justify-between">
                <strong className="text-black-700">{review.name}</strong>
                <p className="text-black-700">
                  {review.createdAt.substring(0, 10)}
                </p>
              </div>
              <p className="my-4 text-black-700">{review.comment}</p>
              <button
